...
function result(){
	...
	//Number isNaN to validate the input is number 
		if(!isNaN (adult)){
			if(!isNaN(child)){
				if(!isNaN(senior)){
					...
					document.getElementById("subtotal").innerHTML=" Your Sub-Total is $" + total;
				}else{
		...
		if(parking!='')
		{
			total=(total)+parseInt(parking);
			document.getElementById("totalResult").innerHTML=" Total Amount is $" + total;
		}
